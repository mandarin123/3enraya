    ...
    <div className="board">
      ...
      <div className="board__body">
        {squares.map((sq, index) => (
          <BoardCard key={index} index={index} user={sq} active={ winner && winnerLine && winnerLine.includes(index) } />
        ))}
      </div>
      <div className="board__footer">
        <div className="card bg-blue">
          <p className="text-light"> x {checkUser('x')}</p>
          <strong className="text-2x1">{ ties.x }</strong>
        </div>
        <div className="card bg-light">
          <p className="text-light"> ties </p>
          <strong className="text-2x1">{ ties.x + ties.o }</strong>
        </div>
        <div className="card bg-blue">
          <p className="text-light"> o {checkUser('o')}</p>
          <strong className="text-2x1">{ ties.o }</strong>
        </div>
      </div>
    </div>
  );
